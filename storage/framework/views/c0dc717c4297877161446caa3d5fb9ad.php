...
<?php foreach($attributes->onlyProps(['permohonans']) as $__key => $__value) {
    $$__key = $$__key ?? $__value;
} ?>
<?php $attributes = $attributes->exceptProps(['permohonans']); ?>
<?php foreach (array_filter((['permohonans']), 'is_string', ARRAY_FILTER_USE_KEY) as $__key => $__value) {
    $$__key = $$__key ?? $__value;
} ?>
...
<div class="bg-white rounded-2xl border border-slate-200/80 shadow-xs overflow-hidden">
    ...
    <div class="p-6 border-b border-slate-100">
        <form id="filter-search-form" action="<?php echo e(route('admin.permohonan.index')); ?>" method="GET" class="flex flex-col lg:flex-row lg:items-center justify-between gap-3.5 w-full">
            ...
            <div class="flex flex-wrap items-center gap-2.5">
                ...
                <div class="flex items-center gap-1 bg-slate-100/80 p-1 rounded-xl border border-slate-200/50 overflow-x-auto max-w-full">
                    <a href="<?php echo e(route('admin.permohonan.index')); ?>" 
                    ...
                    <a href="<?php echo e(route('admin.permohonan.index', array_merge(request()->query(), ['status' => 'Diajukan']))); ?>" 
                    ...
                    <a href="<?php echo e(route('admin.permohonan.index', array_merge(request()->query(), ['status' => 'Diproses']))); ?>" 
                    ...
                    <a href="<?php echo e(route('admin.permohonan.index', array_merge(request()->query(), ['status' => 'Selesai']))); ?>" 
                    ...
                    <a href="<?php echo e(route('admin.permohonan.index', array_merge(request()->query(), ['status' => 'Ditolak']))); ?>" 
                       class="px-3 py-1.5 rounded-lg text-xs md:text-sm transition-all duration-200 whitespace-nowrap <?php echo e(request('status') == 'Ditolak' ? 'bg-white text-slate-900 shadow-xs font-black' : 'text-slate-500 font-bold hover:text-slate-900'); ?>">
                        Ditolak
                    </a>
                </div>
            </div>
            ...
            <div class="flex items-center gap-2 flex-1 lg:max-w-[480px] w-full">
                <div class="flex-1 min-w-0 h-10 relative">
                    <input type="text" name="search" value="<?php echo e(request('search')); ?>" placeholder="Cari no. tiket, nama, NIK pemohon..." 
    ...
    <form id="form-bulk-delete" action="<?php echo e(route('admin.permohonan.bulk')); ?>" method="POST">
        <?php echo csrf_field(); ?>
        ...
        <div class="overflow-x-auto">
            <table class="w-full text-left border-collapse">
                ...
                <tbody class="divide-y divide-slate-100 text-xs md:text-sm">
                    <?php $__empty_1 = true; $__currentLoopData = $permohonans; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $item): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
                        <tr class="hover:bg-sky-50/70 transition-colors">
                            <td class="col-checkbox-cell hidden px-4 py-2.5 text-center">
                                <input type="checkbox" name="ids[]" value="<?php echo e($item->id); ?>" onclick="updateBulkState()" class="item-checkbox w-4 h-4 rounded border-slate-300 text-slate-900 focus:ring-slate-900 cursor-pointer">
                            </td>
                            ...
                            <td class="px-6 py-2.5">
                                <div class="text-xs md:text-sm font-extrabold text-slate-900"><?php echo e($item->nama_lengkap); ?></div>
                                <div class="text-xs text-slate-400 font-semibold">NIK: <?php echo e($item->no_identitas ?? '-'); ?></div>
                            </td>
                            ...
                            <td class="px-6 py-2.5 min-w-[14rem] max-w-md">
                                <p class="text-slate-700 font-medium leading-relaxed whitespace-normal break-words text-xs md:text-sm">
                                    <?php echo e($item->informasi_yang_diminta); ?>

                                </p>
                            </td>
                            <td class="px-6 py-2.5 text-center whitespace-nowrap">
                                <div class="flex flex-col items-center gap-1">
                                    <?php if($item->status === 'Menunggu' || $item->status === 'Diajukan'): ?>
                                    ...
                                    <?php elseif($item->status === 'Terima' || $item->status === 'Selesai'): ?>
                            ...
                            <td class="px-6 py-2.5 text-center whitespace-nowrap">
                                <div class="flex items-center justify-center gap-2">
                                    ...
                                    <?php elseif($statusAwal === 'Diajukan' || $statusAwal === 'Menunggu'): ?>
                                        <a href="<?php echo e(route('admin.permohonan.show', $item->id)); ?>"
                                    ...
                                    <button type="button" onclick="triggerDelete('<?php echo e(route('admin.permohonan.destroy', $item->id)); ?>', '<?php echo e($item->no_tiket); ?>')"
...
<?php $component = Illuminate\View\AnonymousComponent::resolve(['view' => 'components.ui.pagination','data' => ['paginator' => $permohonans,'label' => 'permohonan']] + (isset($attributes) && $attributes instanceof Illuminate\View\ComponentAttributeBag ? (array) $attributes->getIterator() : [])); ?>
...
<?php endif; ?>
<?php $component->withAttributes(['paginator' => \Illuminate\View\Compilers\BladeCompiler::sanitizeComponentAttribute($permohonans),'label' => 'permohonan']); ?>
...
<?php /**PATH D:\laragon\www\ppid-fmipa-baru\resources\views/components/admin/permohonan/table.blade.php ENDPATH**/ ?>