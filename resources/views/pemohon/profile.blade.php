        ...
        <form id="profileForm" action="{{ route('user.profile.update') }}" method="POST" class="space-y-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                    ...
                    <input type="text" name="nama_lengkap" value="{{ old('nama_lengkap', auth()->user()->nama_lengkap) }}" class="w-full px-4 py-3 rounded-2xl border border-slate-300 focus:border-blue-900 focus:ring-2 focus:ring-blue-900/20 outline-none transition-all text-sm font-medium">
            ...
            <div class="border-t border-slate-100 pt-6 space-y-6">
                <h3 class="font-bold text-slate-900 text-md flex items-center gap-2">
                    <i class="fa-solid fa-shield-halved text-blue-900"></i> Keamanan Akun
                </h3>
                
                @if(auth()->user()->password)
                    <div>
                        <label class="block text-xs font-bold text-slate-700 mb-2 uppercase tracking-wider">Password Lama</label>
                        <input type="password" name="current_password" placeholder="Masukkan password saat ini" class="w-full px-4 py-3 rounded-2xl border border-slate-300 focus:border-blue-900 focus:ring-2 focus:ring-blue-900/20 outline-none transition-all text-sm font-medium">
                        @error('current_password') <p class="text-red-500 text-xs mt-1 font-medium">{{ $message }}</p> @enderror
                    </div>
                @else
                    <div class="p-4 bg-blue-50/70 border border-blue-200/50 rounded-2xl text-blue-800 text-xs flex items-start gap-3">
                        <i class="fa-solid fa-circle-info mt-0.5 text-base text-blue-600 shrink-0"></i>
                        <div>
                            <p class="font-bold text-blue-900">Akun Terhubung dengan Google</p>
                            <p class="mt-1 text-blue-700 leading-relaxed">Anda masuk menggunakan Google Sign-In dan belum menyetel kata sandi manual. Anda dapat mengisi kolom **Password Baru** secara langsung untuk membuat kata sandi baru untuk akun Anda.</p>
                        </div>
                    </div>
                @endif
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                        ...
                        <input type="password" name="password" placeholder="Masukkan password baru" class="w-full px-4 py-3 rounded-2xl border border-slate-300 focus:border-blue-900 focus:ring-2 focus:ring-blue-900/20 outline-none transition-all text-sm font-medium">
                    </div>
                    <div>
                        <label class="block text-xs font-bold text-slate-700 mb-2 uppercase tracking-wider">Konfirmasi Password Baru</label>
                        <input type="password" name="password_confirmation" placeholder="Ulangi password baru" class="w-full px-4 py-3 rounded-2xl border border-slate-300 focus:border-blue-900 focus:ring-2 focus:ring-blue-900/20 outline-none transition-all text-sm font-medium">
                    </div>
                </div>
            ...
            <div class="pt-4">
                <button type="button" id="btnSubmit" class="bg-blue-900 text-white px-8 py-3.5 rounded-2xl font-bold hover:bg-blue-800 transition-all shadow-md hover:-translate-y-0.5">
                    Simpan Perubahan
                </button>
            </div>
        </form>
...
<div id="confirmModal" class="hidden fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm">
    <div class="bg-white rounded-3xl p-8 w-full max-w-md shadow-2xl border border-slate-100 transform transition-all duration-200">
        <div class="flex items-center gap-4 mb-4 text-blue-900">
        ...
        <div class="flex gap-3">
            ...
            <button id="confirmBtn" class="flex-1 px-4 py-3 rounded-2xl text-sm font-bold text-white bg-blue-900 hover:bg-blue-800 transition-all shadow-md">Ya, Simpan</button>
        </div>
    </div>
</div>
