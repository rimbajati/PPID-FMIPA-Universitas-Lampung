...
<div id="modal-form" class="fixed inset-0 bg-slate-900/50 hidden z-50 flex items-center justify-center p-4">
    <div class="bg-white rounded-2xl w-full max-w-4xl max-h-[90vh] overflow-y-auto shadow-xl">
        ...
        <form action="{{ route('layanan.store') }}" method="POST" enctype="multipart/form-data" class="p-6 space-y-5" novalidate>
            @csrf
            ...
            <div id="section-keberatan-pilih" class="mb-6 {{ old('jenis_layanan') == 'Keberatan' ? '' : 'hidden' }}">
                @if($permohonans->isEmpty())
                    <div class="p-5 bg-amber-50 border border-amber-200/80 rounded-2xl text-amber-900 text-sm shadow-sm">
                        <div class="flex items-start gap-3.5">
                            ...
                            <div>
                                <h4 class="font-bold text-amber-950 text-base">Belum Ada Riwayat Permohonan Informasi</h4>
                                <p class="mt-1 text-xs text-amber-800 leading-relaxed font-medium">
                                    Pengajuan Keberatan hanya dapat dilakukan terhadap Permohonan Informasi yang telah Anda ajukan sebelumnya. Saat ini Anda belum memiliki riwayat permohonan informasi publik.
                                </p>
                                <button type="button" onclick="switchToPermohonan()" class="mt-3.5 inline-flex items-center gap-2 px-4 py-2.5 bg-amber-600 hover:bg-amber-700 text-white rounded-xl text-xs font-bold transition-all shadow-md">
                                    <i class="fa-solid fa-plus"></i> Buat Permohonan Informasi Baru
                                </button>
                            </div>
                        </div>
                    </div>
            ...
            <div id="form-body-wrapper" class="space-y-6 {{ old('jenis_layanan') == 'Permohonan' || (old('jenis_layanan') == 'Keberatan' && old('permohonan_terkait_id')) ? '' : 'hidden' }}">
                    ...
                    <div>
                        ...
                        <div class="space-y-3.5 pl-1 mb-2">
                            <label class="flex items-center gap-3 cursor-pointer group">
                                <input type="radio" name="cara_ambil_radio" value="Mengambil langsung ke FMIPA" class="w-5 h-5 text-[#1B365D] border-slate-350 focus:ring-[#1B365D] cursor-pointer" {{ old('cara_ambil') == 'Mengambil langsung ke FMIPA' ? 'checked' : '' }} onchange="selectCaraAmbil(this.value)">
                            ...
                            <label class="flex items-center gap-3 cursor-pointer group">
                                <input type="radio" name="cara_ambil_radio" value="Melalui Email atau Website" class="w-5 h-5 text-[#1B365D] border-slate-350 focus:ring-[#1B365D] cursor-pointer" {{ old('cara_ambil') == 'Melalui Email atau Website' ? 'checked' : '' }} onchange="selectCaraAmbil(this.value)">
                ...
                <div id="section-keberatan" class="{{ old('jenis_layanan') == 'Keberatan' ? '' : 'hidden' }} space-y-4">
                    ...
                    <div class="space-y-1">
                        <label class="block text-base font-bold text-slate-700">Lampiran Data Pendukung (Opsional)</label>
                        <input type="file" name="lampiran_pendukung" id="lampiran_pendukung_keberatan" class="input-field w-full border {{ $errors->has('lampiran_pendukung') ? 'border-red-500' : 'border-slate-300' }} rounded-xl p-2 text-sm text-slate-500 file:mr-4 file:py-2 file:px-4 file:rounded-lg file:border-0 file:text-xs file:font-bold file:bg-[#1B365D] file:text-white hover:file:bg-[#1B365D] file:transition-all cursor-pointer">
                        ...
                        <div id="keberatan-existing-file-info" class="mt-2 text-sm text-slate-600 bg-slate-50 p-2.5 rounded-lg border border-slate-100 flex items-center justify-between hidden">
                            <span class="flex items-center gap-1.5 text-xs">
                                <i class="fa-solid fa-file text-[#1B365D] text-sm"></i>
                                <span>Terdapat file pendukung bawaan dari permohonan terkait.</span>
                            </span>
                            <a id="keberatan-existing-file-link" href="#" target="_blank" class="text-[#1B365D] font-semibold hover:underline text-xs flex items-center gap-1">
                ...
                <div class="flex items-center gap-3 p-4 bg-slate-50 rounded-xl border {{ $errors->has('pernyataan') ? 'border-red-500' : 'border-slate-200' }}">
                    <input type="checkbox" name="pernyataan" id="pernyataan" value="1" data-required class="w-5 h-5 rounded border-gray-300 text-[#1B365D]" {{ old('pernyataan') ? 'checked' : '' }}>
                ...
                <button type="submit" class="w-full bg-[#1B365D] text-white py-4 rounded-xl font-bold text-lg hover:bg-[#1B365D] transition-all shadow-xl">Kirim Pengajuan</button>
            </div>
        </form>
    </div>
</div>
