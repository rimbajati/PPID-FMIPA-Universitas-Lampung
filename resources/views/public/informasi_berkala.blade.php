...
<div class="container mx-auto px-4 py-12 pt-32 max-w-7xl">
    ...
    <div class="mb-8">
        <h1 class="text-3xl font-bold text-gray-900 mb-2">Informasi Tersedia Secara Berkala</h1>
        <p class="text-gray-600 max-w-2xl text-sm md:text-base">
            Daftar informasi yang diperbarui dan disediakan secara rutin (berkala) oleh FMIPA Unila untuk masyarakat.
        </p>
    </div>

    <!-- Search Form -->
    <form id="searchForm" action="{{ url('/informasi-berkala') }}" method="GET" class="bg-white p-4 md:p-6 border border-gray-100 shadow-xl mb-8 flex flex-col md:flex-row gap-4 items-center rounded-3xl">
        ...
        <div class="overflow-x-auto w-full">
            <table class="w-full min-w-[700px] text-left border-collapse">
                ...
                <tbody class="divide-y divide-gray-300">
                    @forelse ($informasi->groupBy('rincian_informasi') as $grup => $items)
                        @foreach($items as $index => $item)
                            <tr class="hover:bg-gray-50 transition">
                                {{-- Hanya tampilkan sel Rincian Informasi di baris pertama dari grup tersebut --}}
                                @if($index === 0)
                                ...
                                <td class="p-5 text-center">
                                    <a href="{{ url('/informasi/akses/'.$item->id) }}"
                                        target="_blank"
                    ...
                    @empty
                        <tr>
                            <td colspan="4" class="p-10 text-center text-gray-400">Data tidak ditemukan.</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
