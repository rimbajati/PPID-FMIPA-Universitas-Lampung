...
<div class="max-w-4xl mx-auto">
    ...
    @if (session('status'))
        <div id="notifBox" class="mb-6 p-4 bg-green-50 border-l-4 border-green-500 rounded-lg shadow flex items-center justify-between">
        ...
        <form id="profileForm" action="{{ route('user.profile.update') }}" method="POST" class="space-y-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                    ...
                    <input type="text" name="nama_lengkap" value="{{ old('nama_lengkap', auth()->user()->nama_lengkap) }}" class="w-full px-4 py-2.5 rounded-lg border border-gray-300 focus:ring-2 focus:ring-[#0a192f] outline-none">
                ...
                <div>
                    ...
                    <input type="email" value="{{ auth()->user()->email }}" disabled class="w-full px-4 py-2.5 rounded-lg border border-gray-200 bg-gray-50 text-gray-500 cursor-not-allowed">
                </div>
            </div>
            ...
            <div class="border-t border-gray-100 pt-6 space-y-4">
                ...
                <div>
                    ...
                    <input type="password" name="current_password" placeholder="Masukkan password saat ini" class="w-full px-4 py-2.5 rounded-lg border border-gray-300 focus:ring-2 focus:ring-[#0a192f] outline-none">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                        ...
                        <input type="password" name="password" placeholder="Masukkan password baru" class="w-full px-4 py-2.5 rounded-lg border border-gray-300 focus:ring-2 focus:ring-[#0a192f] outline-none">
                    </div>
                    <div>
                        <label class="block text-xs font-bold text-gray-700 mb-2">Konfirmasi Password Baru</label>
                        <input type="password" name="password_confirmation" placeholder="Ulangi password baru" class="w-full px-4 py-2.5 rounded-lg border border-gray-300 focus:ring-2 focus:ring-[#0a192f] outline-none">
                    </div>
                </div>
                @error('password') <p class="text-red-500 text-[10px] mt-1">{{ $message }}</p> @enderror
            </div>

            <button type="button" id="btnSubmit" class="bg-[#0a192f] text-white px-8 py-3 rounded-xl font-bold hover:bg-gray-800 transition shadow-sm">
                SIMPAN PERUBAHAN
            </button>
        </form>
    </div>
...
<div id="confirmModal" class="hidden fixed inset-0 z-50 flex items-center justify-center p-4 bg-black bg-opacity-50">
    <div class="bg-white rounded-2xl p-6 w-full max-w-sm shadow-xl">
        ...
        <div class="flex gap-3">
            <button id="cancelBtn" class="flex-1 px-4 py-2 rounded-xl text-sm font-bold text-gray-600 bg-gray-100 hover:bg-gray-200">Batal</button>
            <button id="confirmBtn" class="flex-1 px-4 py-2 rounded-xl text-sm font-bold text-white bg-[#0a192f] hover:bg-gray-800">Ya, Simpan</button>
        </div>
    </div>
</div>
