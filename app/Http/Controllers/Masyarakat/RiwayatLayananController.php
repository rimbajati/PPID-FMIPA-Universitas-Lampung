...
class RiwayatLayananController extends Controller
{
    ...
    public function index()
    {
        ...
        $permohonans = Permohonan::where('user_id', $userId)->latest()->get()->map(function ($item) use ($user) {
            ...
            // Parsing catatan_admin JSON jika ada
            $notes = json_decode($item->catatan_admin, true);
            if (is_array($notes)) {
                $item->catatan_verifikasi = $notes['verifikasi'] ?? null;
                $item->catatan_proses     = $notes['proses'] ?? null;
                $item->catatan_final      = $notes['final'] ?? null;
            } else {
                $item->catatan_final      = $item->catatan_admin;
            }
        ...
        $keberatans = Keberatan::with('permohonan')->where('user_id', $userId)->latest()->get()->map(function ($item) use ($user) {
            ...
            $item->identitas_file = $item->permohonan ? $item->permohonan->identitas_file : null;
            $item->identitas_file_url = $item->identitas_file ? asset('storage/' . $item->identitas_file) : null;
            $item->file_pendukung_url = $item->pendukung_file ? asset('storage/' . $item->pendukung_file) : null;
